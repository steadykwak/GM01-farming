...
function searchInfo() {
    ...
    fetch(`${API_BASE}?action=getstudentinfo&name=${encodeURIComponent(name)}&phone=${encodeURIComponent(phone)}`)
        .then((res) => res.json())
        .then(showResult)
        .catch((err) => {
...
function showResult(res) {
    ...
    const progress = Math.min(100, Math.round(((res.exp - prevExp) / (nextExp - prevExp)) * 100));
